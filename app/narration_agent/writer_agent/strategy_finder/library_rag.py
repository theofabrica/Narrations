...
class LibraryRAG:
    ...
    def retrieve(self, context_pack: Dict[str, Any], limit: int = 3) -> List[Dict[str, Any]]:
        if not isinstance(context_pack, dict):
            return []
        if not self._client:
            return self._fallback_local(context_pack, limit)
        return self._retrieve_agentic(context_pack, limit)
    ...
    def _retrieve_agentic(self, context_pack: Dict[str, Any], limit: int) -> List[Dict[str, Any]]:
        ...
        task_prompt = _build_task_prompt(context_pack, limit)
    ...
    def _fallback_local(self, context_pack: Dict[str, Any], limit: int) -> List[Dict[str, Any]]:
        ...
        for item in items:
            ...
            if writing_typology and writing_typology not in (typologies or []):
                continue
